...
describe('useInformationLogic', () => {
    ...
    test('noData is true when places is empty', () => {
        const props = { ...defaultProps, places: [] };
        const { getByTestId } = renderWithProviders(<TestComponent {...props} />);
        expect(getByTestId('noData').textContent).toBe('true');
    });
});
