...
describe('AuthContext', () => {
    ...
    test('should set token correctly', () => {
        ...
        // Simula clique no botão para alterar token
        fireEvent.click(screen.getByText('Set Token'));

        // Verifica que token foi atualizado no DOM e localStorage
    ...
    test('should remove token from localStorage when setToken is called with null', () => {
        localStorage.setItem('token', 'existingToken');

        // Executa setToken(null) diretamente via hook
        renderWithAuthProvider(({ setUsername }) => {
            setUsername(null);
        });
